...
DYLIBS=`otool -L cryonote-qt | grep /usr/local | grep -v :$ | awk -F' ' '{ print $1 }'`
...
# replace cryonote-qt links and library links
for dylib in $DYLIBS; do
echo Rewriting `basename $dylib`...
install_name_tool -change $dylib @executable_path/lib/`basename $dylib` cryonote-qt;
