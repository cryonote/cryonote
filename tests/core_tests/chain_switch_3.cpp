...
bool gen_chainswitch_txin_to_key::generate(std::vector<test_event_entry>& events) const
{
  ...
  MAKE_TX_MIX_CP_FEE(events, tx_to_bob, alice, bob, 99, 0, blk_2, CP_XCN, 0);
  ...
  MAKE_TX_MIX_CP_FEE(events, tx_to_bob_2, alice, bob, 66, 0, b_blk_4, CP_XCN, 0);
  
  SET_EVENT_VISITOR_SETT(events, event_visitor_settings::set_txs_keeped_by_block, true);
  MAKE_TX_MIX_CP_FEE(events, tx_to_bob_3, alice, bob, 66, 0, b_blk_4, CP_XCN, 0);
...
bool gen_chainswitch_contract_create_send::generate(std::vector<test_event_entry>& events) const
{
  ...
  DONT_MARK_SPENT_TX(events);
  MAKE_TX_MIX_CP_FEE(events, tx_alice_spend, alice, carol, 67, 0, blk_5, CP_XCN, 0)
  // bob can spend floor(33*.25) = 8 - ceil(8*.07) = 7
  //             + floor(66*.25) = 16 - ceil(16*.07) = 14
  DONT_MARK_SPENT_TX(events);
  MAKE_TX_MIX_CP_FEE(events, tx_bob_spend, bob, carol, 21, 0, blk_5, CP_XCN, 0)
  // carol can spend floor(99*.07) = 6
  DONT_MARK_SPENT_TX(events);
  MAKE_TX_MIX_CP_FEE(events, tx_fee_spend, carol, alice, 6, 0, blk_5, CP_XCN, 0)
  ...
  GRADE_CONTRACT_TX_FULL(events, tx_grade2, 0xbe7, GRADE_SCALE_MAX / 4, grading_key.sec,
                         carol, 2, CURRENCY_XCN, tools::identity());
  ...
  DO_CALLBACK(events, "mark_invalid_tx");
  MAKE_TX_MIX_CP_FEE(events, tx_alice_spend2_bad, alice, carol, 22, 0, c_blk_15, CP_XCN, 0);
  
  SET_EVENT_VISITOR_SETT(events, event_visitor_settings::set_txs_keeped_by_block, true);
  // should work if we force it though...
  MAKE_TX_MIX_CP_FEE(events, tx_alice_spend2, alice, carol, 22, 0, c_blk_15, CP_XCN, 0);
  // bob can spend floor(33*.25) = 8 - ceil(8*.07) = 7
  MAKE_TX_MIX_CP_FEE(events, tx_bob_spend2, bob, carol, 7, 0, c_blk_15, CP_XCN, 0);
  // carol can spend floor(33*.07) = 2
  MAKE_TX_MIX_CP_FEE(events, tx_fee_spend2, carol, alice, 2, 0, c_blk_15, CP_XCN, 0);
...
bool gen_chainswitch_contract_grade::generate(std::vector<test_event_entry>& events) const
{
  ...
  GRADE_CONTRACT_TX_FULL(events, tx_grade, 0xbe7, GRADE_SCALE_MAX / 4, grading_key.sec,
                         carol, 0, CURRENCY_XCN, tools::identity());
  DO_CALLBACK(events, "mark_head_1");
